...
class StyleTransferModel():
    ...
    def run_epoch(self, epoch):
        ...
        for idx, batch in tqdm(enumerate(self.train_loader), total=len(self.train_loader)):
            ...
            if self.args.add_noise:
                encoder_output_detached = self.add_noise(encoder_output_detached, epoch, self.args.noise_std)
            ...
            ae_loss = rec_loss
            if epoch > self.args.ae_pretraining_epochs:
                ae_loss += self.args.lambda_gan * enc_loss
            ...
            if epoch > self.args.ae_pretraining_epochs: # should be epoch > pre-training-epochs
    ...
    def add_noise(self, x, epoch, std=0.2):
        return x + std*(1 - epoch/self.args.epochs) * torch.randn_like(x)
